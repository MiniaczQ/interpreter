use crate::{
    lexer::lexem::{Lexem, LexemBuilder, LexemType, LexerWarningVariant},
    scannable::Scannable,
};
...
fn escape_characters(lb: &mut LexemBuilder, content: &mut Vec<char>) {
    match lb.curr() {
        ...
        '\\' => content.push('\\'),
        c => {
            lb.error(LexerWarningVariant::InvalidEscapeCharacter(c));
        }
    }
}
...
fn complete_string(lb: &mut LexemBuilder, max: usize) -> Lexem {
    ...
    loop {
        ...
        match lb.curr() {
            ...
            '\x03' => {
                lb.error(LexerWarningVariant::StringNeverEnds);
        ...
        if content.len() > max {
            content.pop();
            lb.error(LexerWarningVariant::StringTooLong);
            break lb.bake_raw(LexemType::String(content.into_iter().collect()));
        }
        lb.pop();
    }
